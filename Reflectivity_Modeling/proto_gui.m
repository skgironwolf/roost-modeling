...
axes(handles.axes4);  
    [x,y,Z] = genRoost(handles.x0, handles.y0, handles.radius, handles.a);
    % Display with color scale that goes from 0 to 1
    imagesc(x, y, Z, [-5 30]); %[0 handles.a*1.75]
...
% get roost and display;
    [roo,radar,DZ] = getRoost(handles.scani,handles.seqi);
    [X2,Y2,X,Y,Z2] = getDisplay(radar,roo);
    [Y3, X3] = ndgrid(Y2, X2);
    handles.zf = 1;
    ...
    Y = Y(~isnan(dbZ));
    X = X(~isnan(dbZ));
    loss = lossF2([handles.x0,handles.y0,handles.radius,handles.a],X,Y,dbZ);
%     set(handles.text14,'String',num2str(loss));
    % show the image
    imagesc(X2, Y2, Z2,[-5 30]);
    colorbar;
    [V,UV,VV,vr] = getVelocity(roo.x,roo.y,OX,OY,handles.windspeed,handles.birdspeed);
    hold on; 
    quiver(OX,OY,UV,VV,2);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    guidata(hObject,handles);
    roo = handles.roo;
    border2 = handles.OX < (roo.x+(roo.r+(roo.r))*(handles.zf)) & handles.OX > (roo.x-(roo.r+(roo.r))*(handles.zf)) & handles.OY < (roo.y+(roo.r+(roo.r))*(handles.zf)) & handles.OY > (roo.y-(roo.r+(roo.r))*(handles.zf));
       
if(-150000 < min(handles.OX(border2)) && -150000 < min(handles.OY(border2)) && 150000 > max(handles.OX(border2)) && 150000 > max(handles.OY(border2)))
    
    xlimit = [min(handles.OX(border2)) max(handles.OX(border2))];
    ylimit = [min(handles.OY(border2)) max(handles.OY(border2))];
    xlim(handles.axes6,xlimit);
    ylim(handles.axes6,ylimit);
    xlim(handles.axes4,xlimit);
    ylim(handles.axes4,ylimit);
   fprintf('print out zoom out');
else
