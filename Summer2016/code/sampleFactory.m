...
nRoosts = 10; %numel(roosts); % number of roosts 
array = [roosts.scan_id];
% fid=fopen('/Users/saadia/Desktop/wsrlib/info_VR.dat','w');
...
% %% scale DZ. map range [-20 20] into [0 1], then set NaN to 0
% dzlim = [-20 20];
...
file = sprintf('%s%d%s%d%s','/positiveSamples/',roo.scan_id,'-gray-',roo.sequence_id,'.png');
% fprintf(fid, '%s %d %d %d %d %d \n', file, [ 1 RECT(1) RECT(2) RECT(3) RECT(4)]); %size(array(array == roo.scan_id),2)
% imwrite(VR,sprintf('%s%d%s%d%s','/positiveSamples/',roo.scan_id,'-gray-',roo.sequence_id,'.png'));
% %pause;
data(i).imageFilename = file;
data(i).objectBoundingBoxes = RECT;
info{i} = data(i); 
end
