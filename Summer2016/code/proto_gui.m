...
%sqrt(uwind^2+vwind^2)%print windspeed
%birdshift(roo,handles.scani,handles.seqi)%print birdspeed
%currently I have the wind set to 0 
% [V,UV,VV,vr] = getVelocity(roo.x,roo.y,X,Y,0,0,birdshift(roo,handles.scani,handles.seqi));%handles.birdspeed
[V,UV,VV,vr] = getVelocity(roo.x,roo.y,X,Y,uwind,vwind,handles.birdspeed);%birdshift(roo,handles.scani,handles.seqi)
...
% imwrite(image, vrmap2(32),'roost.png');

%Save Entire Image 
%imwrite(im.CData,vrmap2(32),'roost.png');
...
plot(xunit, yunit,'LineWidth', 5,'color', 'red');
  ...
  G = zeros(2,3);
    ...
    G(1,2) = handles.y0;
    G(2,2) = handles.roo.y;
    G(1,3) = handles.radius;
    G(2,3) = handles.roo.r;
    area_intersect_circle_analytical(G)
end
